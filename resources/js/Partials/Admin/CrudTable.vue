...
export default { 
  ...
  setup (props, ctx) {
    ...
    const processDialog = (payload)=>{}
    const saveItem = (payload)=>{}
    const loadItems = (payload)=>{}
    
    return { ...toRefs(state), processDialog, saveItem, loadItems  }
  }
} 
</script>
...
<template>
  <div class="container">
    <v-data-table multi-sort dense show-select class="elevation-1" :single-select="false" :headers="headers" :search="search" :items="items" sort-by="items" item-key="id" v-model="selected">
