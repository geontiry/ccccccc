<template>
  <v-card id="account-setting-card">
    ...
    <v-tabs v-model="tab" show-arrows>
      <v-tab v-for="tab in tabs" :key="tab.icon"><v-icon size="20" class="me-3">{{ tab.icon }}</v-icon><span>{{ tab.title }}</span></v-tab>
    </v-tabs>
...
export default {
  ...
  setup() {
    ...
    const accountSettingData = {
      account: {
        avatarImg: 'storage/assets/images/avatars/1.png'.default,
        username: 'johnDoe',
        name: 'john Doe',
