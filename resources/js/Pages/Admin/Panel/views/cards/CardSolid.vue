...
export default{
  ...
  setup (props, ctx) {
    ...
    return { ...toRefs(state), solidCardData } 
  }
} 
</script>  
...
<template>
  <v-row>
    <v-col v-for="data in solidCardData" :key="data.icon" cols="12" md="6" lg="4">
      <v-card :color="data.cardBg" dark>
        <v-card-title>
          <v-icon size="1.5rem" color="white" class="me-2">{{ data.icon }}</v-icon>
          <span class="white--text">{{ data.title }}</span>
        </v-card-title>
        <v-card-text class="white--text">{{ data.text }}</v-card-text>
        <v-card-actions class="justify-space-between">
          <div>
            <v-avatar size="34">
              <v-img :src="`storage/assets/images/avatars/${data.avatarImg}`"></v-img>
            </v-avatar>
            <span class="white--text ms-2">{{ data.avatarName }}</span>
          </div>
          <div class="white--text">
            <v-icon class="me-1" color="white" size="1rem"> mdi-heart</v-icon>
            <span class="text-sm me-4">{{ data.likes }}</span>
            <v-icon class="me-1" color="white" size="1rem">mdi-share-variant</v-icon>
