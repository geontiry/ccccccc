<script>
import Navbar from '@/Pages/Brenden/EBookshop/Navbar.vue'
...
export default{
  ...
  setup (props, ctx) {
    ...
    onMounted(() => setTimeout(() => { loading = false; searchTextsLoading = false}, 2000))
              ...
              <v-list>
                <v-list-item-group>
                  <template v-for="(item, i) in items">
                    ...
                    <v-list-item active-class="grey lighten-4" :key="`item-${i}`" class="py-2" @click="clickItem(item)">
                      ...
                      <v-list-item-action><v-radio :key="item" :value="item"></v-radio></v-list-item-action>
                    </v-list-item>
                  </template>
                </v-list-item-group>
              </v-list>
                ...
                <v-list-item-group>
                  <v-list-item v-for="(link, i) in links" :key="i">
                    <v-list-item-icon><v-icon v-text="link.icon"></v-icon></v-list-item-icon>
                    <v-list-item-content><v-list-item-title class="text-uppercase">{{link.text}} {{ historyType }}</v-list-item-title></v-list-item-content>
                  </v-list-item>
                </v-list-item-group>
              </v-list>
          </v-card>
