      ...
      <template v-slot:activator="{ on: menu, attrs }">
        <v-tooltip left>
          <template v-slot:activator="{ on: tooltip }">
            <v-btn small fab dark fixed bottom right color="primary" v-bind="attrs" v-on="{ ...tooltip, ...menu }" class="mr-12" @click="shown = !shown">
              <v-badge left color="deep-purple accent-4" offset-x="4" offset-y="12" content="5"><v-icon>{{ shown ? 'mdi-progress-close' : 'mdi-chat' }}</v-icon></v-badge>
              </v-btn>
          </template>
